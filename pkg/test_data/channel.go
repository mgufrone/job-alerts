...
func randomHour() string {
	return faker.RandomChoice([]string{"*", "0", "1/5", "5,10,15,59"})
}
...
func ValidChannel() *channel.Entity {
	var usr user.Entity
	usr.SetID(uuid.New())
	title := faker.Company().Name()
	desc := faker.Lorem().Paragraph(5)
	activeS := faker.RandomChoice([]string{"true", "false"})
	active, _ := strconv.ParseBool(activeS)
