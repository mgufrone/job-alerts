...
func NewQueryRepository(instance *db.Instance) user.IQueryRepository {
	return &queryRepository{instance: instance}
}

func (q *queryRepository) CriteriaBuilder() criteria.ICriteriaBuilder {
	return db.NewCriteriaBuilder("users")
}

func (q *queryRepository) Apply(cb criteria.ICriteriaBuilder) user.IQueryRepository {
	q.cb = cb
	return q
}

func (q *queryRepository) GetAll(ctx context.Context) (out []*user.Entity, err error) {
	defer q.resetCriteria()
	var jobs []*models.User
...
func (q *queryRepository) GetAndCount(ctx context.Context) (out []*user.Entity, total int64, err error) {
	cr := q.cb
...
func (q *queryRepository) FindByID(ctx context.Context, id uuid.UUID) (out *user.Entity, err error) {
	cr := q.CriteriaBuilder().Where(user.WhereID(id))
