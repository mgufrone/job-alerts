...
func NewQueryRepository(instance *db.Instance) user_channel.IQueryRepository {
	return &queryRepository{instance: instance}
}

func (q *queryRepository) CriteriaBuilder() criteria.ICriteriaBuilder {
	return db.NewCriteriaBuilder("user_channels")
}

func (q *queryRepository) Apply(cb criteria.ICriteriaBuilder) user_channel.IQueryRepository {
	q.cb = cb
	return q
}

func (q *queryRepository) GetAll(ctx context.Context) (out []*user_channel.Entity, err error) {
	defer q.resetCriteria()
	var jobs []*models.UserChannel
...
func (q *queryRepository) GetAndCount(ctx context.Context) (out []*user_channel.Entity, total int64, err error) {
	cr := q.cb
...
func (q *queryRepository) FindByID(ctx context.Context, id uuid.UUID) (out *user_channel.Entity, err error) {
	cr := q.CriteriaBuilder().Where(user_channel.WhereID(id))
